...
assert.equal(ledger.incidents.length, 48);
for (const event of ['pull_request', 'merge_group']) {
  ...
  assert.match(result.stdout, /dispatched 48 bounded/);
}
...
for (const [id, metadata] of Object.entries(registry.incident_metadata)) {
  ...
  assert.ok(['pr', 'post-merge', 'organization-required-workflow', 'runner-canary', 'runner-heartbeat', 'landing-manifest'].includes(metadata.ci_stage_owner), `${id} must declare a valid CI stage owner`);
...
console.log('fresh scaffold dispatch contract passed: two events and 48 negative cases');
