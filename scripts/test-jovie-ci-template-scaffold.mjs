...
assert.equal(ledger.incidents.length, 27);
for (const event of ['pull_request', 'merge_group']) {
  ...
  assert.match(result.stdout, /dispatched 27 bounded/);
}
...
console.log('fresh scaffold dispatch contract passed: two events and 27 negative cases');
