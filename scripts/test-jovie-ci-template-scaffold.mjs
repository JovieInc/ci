...
assert.equal(ledger.incidents.length, 26);
for (const event of ['pull_request', 'merge_group']) {
  ...
  assert.match(result.stdout, /dispatched 26 bounded/);
}
...
console.log('fresh scaffold dispatch contract passed: two events and 26 negative cases');
